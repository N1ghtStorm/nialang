...
use crate::nia_std::{
    GATE_CNOT, GATE_H, GATE_S, GATE_T, GATE_X, GATE_Y, GATE_Z, MEASURE, QUBIT, RECORD,
};
...
fn collect_stmt(
    ...
    match st {
        ...
        | Stmt::For { .. }
        | Stmt::Gpu { .. } => Err(
            "QIR lowering currently supports only `let q = qubit();`, one-qubit gates, `CNOT(c, t);`, `let r = q_measure(q);`, and `q_record(r);` inside `quant` blocks"
                .into(),
        ),
    }
}
